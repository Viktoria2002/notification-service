...
class ListenApplicationRejectedMessages {

  @Value("${spring.kafka.consumer.topics.application-rejected-topic}")
  ...
  @Test
  void listenApplicationRejectedMessages_successfullyMessageReceived() {
    String testString = "test";
    CreditNotificationRequest creditNotificationRequest = createCreditNotificationRequest();

    kafkaTemplate.send(applicationRejectedTopic, creditNotificationRequest);
    kafkaTemplate.flush();

    when(templateLoader.loadApplicationRejectedTemplate(any())).thenReturn(testString);
    when(subjectProperties.getApplicationRejectedSubject()).thenReturn(testString);
    doNothing().when(grpcEmailService).send(any(), any(), any());

    verify(kafkaConsumerListener, timeout(5000)).listenApplicationRejectedMessages(
        captor.capture());
