...
class SendWithdrawnByClientNotification {

  @Value("${spring.kafka.consumer.topics.application-withdrawn-by-client-topic}")
  private String applicationWithdrawnByClientTopic;
  ...
  @Test
  void sendWithdrawnByClientNotification_successfullyMessageReceived() {
    String testString = "test";
    CreditNotificationRequest creditNotificationRequest = createCreditNotificationRequest();

    kafkaTemplate.send(applicationWithdrawnByClientTopic, creditNotificationRequest);
    kafkaTemplate.flush();

    when(templateLoader.loadApplicationWithdrawnByClientTemplate(any())).thenReturn(testString);
    doNothing().when(grpcEmailService).send(any(), any(), any());

    verify(kafkaConsumerListener, timeout(5000)).sendWithdrawnByClientNotification(
        captor.capture());
