package ru.aston.notificationservice.kafka.filer;
...
public class DeserializationRecordFilter<K, V> implements RecordFilterStrategy<K, V> {
  ...
  private final LogAccessor logger = new LogAccessor(
      LogFactory.getLog(DeserializationRecordFilter.class));
  ...
  private void logDeserializationException(ConsumerRecord<K, V> consumerRecord) {
    DeserializationException ex = SerializationUtils.byteArrayToDeserializationException(logger,
        consumerRecord.headers()
            .lastHeader(SerializationUtils.VALUE_DESERIALIZER_EXCEPTION_HEADER));

    logger.error(String.format(
        "Record[%s] at offset [%d] could not been deserialized. Exception: [%s {Message: %s}]",
        new String(ex.getData(), UTF_8), consumerRecord.offset(), ex.getCause(),
